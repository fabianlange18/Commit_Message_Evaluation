...
def t_sne(vectors, data, projects=False, legend=True):
    ...
    while i < len(data) - 1:
        for j in range(i + 1, len(data)):
            if projects:
                if data['project'][i] != data['project'][j] or j == len(data) - 1:
                    ax2.scatter(t_sne_2_prediction[i:j, 0], t_sne_2_prediction[i:j, 1], s=1, label=data['project'][i])
                    ax3.scatter(t_sne_3_prediction[i:j, 0], t_sne_3_prediction[i:j, 1], t_sne_3_prediction[i:j, 2], s=1, label=data['project'][i])
                    i = j
            else:
...
def pca(vectors, data, projects=False):
    ...
    while i < len(data) - 1:
        for j in range(i + 1, len(data)):
            if projects:
                if data['project'][i] != data['project'][j] or j == len(data) - 1:
                    ax2.scatter(pca_2_prediction[i:j, 0], pca_2_prediction[i:j, 1], s=1, label=data['project'][i])
                    ax3.scatter(pca_3_prediction[i:j, 0], pca_3_prediction[i:j, 1], pca_3_prediction[i:j, 2], s=1, label=data['project'][i])
                    i = j
            else:
